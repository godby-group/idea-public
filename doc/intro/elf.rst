...
.. math:: ELF(x) = \frac{1}{1 + \bigg(\frac{D_{\sigma}(x)}{D_{\sigma, H}(x)}\bigg)^{2}}, 

where :math:`D_{\sigma}(x)` characterizes the probability of finding a second electron close to the reference electron, and :math:`D_{\sigma, H}(x)` is the same quantity but for the homogeneous electron gas (HEG). In our code, we provide different ways of calculating ELF(x). 
...
In practical calculations the many-electron wavefunction cannot be calculated and so :math:`D_{\sigma}(x)`, and hence :math:`ELF(x)`, have to be approximated. Becke and Edgecombe approximated :math:`D_{\sigma}(x)` as:
