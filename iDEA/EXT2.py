...
def initial_wavefunction(pm, wavefunction_reduced, ground_state=True):
    ...
    ground_state : bool
        - True: Construct a Slater determinant of the two lowest eigenstates of 
                the non-interacting system to use as the initial wavefunction
    ...
    # Find the two lowest eigenstates of the harmonic oscillator
    else: 
...
def solve_imaginary_time(pm, A_reduced, C_reduced, wavefunction_reduced, 
    ...
    while (i < pm.ext.iimax):
        ...
        if(pm.ext.par == 0):
            ...
            b_reduced = mkl.mkl_mvmultiply_c(C_reduced.data,
                        C_reduced.indptr+1,C_reduced.indices+1,1,
                        wavefunction_reduced,C_reduced.shape[0],
                        C_reduced.indices.size)
