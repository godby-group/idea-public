...
def construct_antisymmetry_matrices(pm):
    ...
    coo_size = int(np.prod(list(range(pm.space.npt,pm.space.npt+3)))\
               /spmisc.factorial(3))
    ...
    coo_1, coo_2 = EXT_cython.reduction_three(coo_1, coo_2, pm.space.npt)
    ...
    reduction_matrix = sps.coo_matrix((coo_data_1,(coo_1,coo_2)), shape=(
                       coo_size,pm.space.npt**3), dtype=np.float)
    expansion_matrix = sps.coo_matrix((coo_data_2,(coo_3,coo_4)), shape=(
                       pm.space.npt**3,coo_size), dtype=np.float)
...
def construct_A_reduced(pm, reduction_matrix, expansion_matrix, td):
    ...
    coo_size = int((3*pm.sys.stencil-2)*(pm.space.npt**3))
    ...
    if(td == 0):
        ...
        A = prefactor*sps.coo_matrix((coo_data,(coo_1,coo_2)), shape=(
            pm.space.npt**3, pm.space.npt**3), dtype=np.float)
        A += sps.identity(pm.space.npt**3, dtype=np.float)
    elif(td == 1):
        coo_data = coo_data.astype(np.cfloat)
        prefactor = 1.0j*pm.sys.deltat/2.0
        A = prefactor*sps.coo_matrix((coo_data,(coo_1,coo_2)), shape=(pm.space.npt**3,
            pm.space.npt**3), dtype=np.cfloat)
...
def solve_imaginary_time(pm, A_reduced, C_reduced, wavefunction_reduced, 
    ...
    while (i < pm.ext.iimax):
        ...
        norm = np.linalg.norm(wavefunction_reduced)*(np.sqrt(pm.space.delta)**3)
    ...
    norm = np.linalg.norm(wavefunction)*(np.sqrt(pm.space.delta)**3)
...
def main(parameters):
    ...
    C_reduced = -A_reduced + 2.0*reduction_matrix*sps.identity(pm.space.npt**3,
    ...
    if(pm.run.time_dependence == True):
        ...
        C_reduced = -A_reduced + 2.0*reduction_matrix*sps.identity(
                    pm.space.npt**3, dtype=np.cfloat)*expansion_matrix
