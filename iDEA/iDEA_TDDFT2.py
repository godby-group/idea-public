...
if int(selection) != 1 and int(selection) != 0:
	sys.exit("Input error. Please, it's not rocket science.")
print
print 'Calculate KS potential from approximate functional?'
answ = raw_input('(Yes=1/No=0):')
if int(answ) != 1 and int(answ) != 0:
	sys.exit("Input error. Please, it's not rocket science.")
...
sqdx = sqrt(dx)
qmax = 1.0										#Maximum value for mixing parameter.
qmin = 1.0										#Minimum value for mixing parameter.
mmax = 10										#Maximum number of iterations.
mu = 1.0
...
Psi2 = zeros(Nx, dtype='complex')
Psi3 = zeros(Nx, dtype='complex')
...
Apot = zeros(Nx)
V_xc_H = zeros((Nt,Nx))									#Hartree and XC potential.
E = zeros((Nt,Nx))
E_xc = zeros((Nt,Nx))
cost_n = zeros(Nt)
U_F = zeros((Nt,Nx))
...
def Hartree(n):
	...
	n_k = momentumspace(n)*dx							#Define function for generating the Hartree 												potential for a given charge density.
	X_x = zeros(Nx)
	for i in range(Nx):
		x = i*dx-0.5*L
		X_x[i] = 1.0/(abs(x)+c)
...
def Potential(j, i, V):									#Potential generator.
	...
	x = i*dx-0.5*L
	if j > 0: 
		V[j,i] += pm.petrb(x)							#Peturbing potential.
	return V[j,i]

#-----------------------------------Reverse-engineering------------------------------

def KohnSham(V, j, Psi0, Psi1, n_x0, J_x0, A, mmax, qmin, cost_U, n_x):			#Kohn-Sham potential for t>0.
	...
	A[j,:] = A[j-1,:]
	n_x, V, Psi0, Psi1, J_x, cost_J[j], cost_n[j] = CrankNicolson(V, n_x0, Psi0\
	, Psi1, j, J_x0, A, n_x)
	...
	q = qmax
	m = 0
	cost_min = 2.0									#Makes sure that the first cost is recorded.
	while cost_n[j] > 0:								#Uses a direct reverse-engineering method.
		...
		n_x, V, Psi0, Psi1, J_x, cost_J[j], cost_n[j] = CrankNicolson(V, n_x0,\
		 Psi0, Psi1, j, J_x0, A, n_x)
		...
		#	l = imaxl-i
		#	if n_x0[j,l] < 1e-5:
		#		cost_U[j] += abs(U_F0[j,i]-U_F[j,i])*dx
		#for i in range(int(0.5*(Nx-1)-imaxr+1)):
		#	l = int(0.5*(Nx-1)+imaxr+i)
		#	if n_x0[j,l] < 1e-5:
		#		cost_U[j] += abs(U_F0[j,i]-U_F[j,i])*dx
		...
		#ufile.write('%s %s\n' % (m,cost_U[j]))					#Keep track of the charge density cost.
		cfile.write('%s %s\n' % (m,cost_J[j]))					#Keeps track of the current density cost.

		if cost_J[j] <= cost_min:
			cost_min = cost_J[j]
			A_min[j,:] = A[j,:]
		if m == mmax:								#Max number of iterations. 
			print 'Does not converge in %s iterations.' % mmax
			print 'Convergance parameter halfed...' 
			q = 0.5*q
			A[j,:] = A[j-1,:]
			m = 0
			n_x, V, Psi0, Psi1, J_x, cost_J[j], cost_n[j] = \
			CrankNicolson(V, n_x0, Psi0, Psi1, j, J_x0, A, n_x)
			if q <= qmin:
				print 'Convergance parameter has reached is minium.' 
				print 'Reverting back to potential corresponding to the lowest overall cost.'
				A[j,:] = A_min[j,:]
				break
		if abs(cost_old-cost_J[j]) < 1e-14:					#Stops if there is no noticable change to 												the system.
			if cost_J[j] == cost_min:
	...
	n_x, V, Psi0, Psi1, J_x, cost_J[j], cost_n[j] = CrankNicolson(V, n_x0\
	, Psi0, Psi1, j, J_x0, A, n_x)
	return n_x, V, cost_J[j], Psi0, Psi1, J_x, A, cost_n[j], cost_U[j]		#Returns the charge density and the 												Kohn-Sham potential.
...
def Groundstate(V, n_x0, j, retflag, mu, setup):					#Uses TISE to find initial ground state 											wave-functions.
	...
	n_x[j,:] = abs(Psi0[j,:])**2+abs(Psi1[j,:])**2					#Charge density composed from the ground 												state and 1st excited state wavefunctions.
	cost_n[j] = sum(abs(n_x0[j,:]-n_x[j,:]))*dx

	if retflag == 0:
		return cost_n[j]							#The cost the optimiser recieves.
	if retflag == 1:
		return n_x, Psi0, Psi1, V, cost_n[j], K, Psi2, Psi3			#Cost function matches the standard 												definition.

#--------------------------Crank Nicolson method evolves TDSE------------------------ 

def CrankNicolson(V, n_x0, Psi0, Psi1, j, J_x0, A, n_x):
	...
	n_x[j,:] = abs(Psi0[j,:])**2+abs(Psi1[j,:])**2
	J_x[j,:] = Currentdensity(j, n_x)
	cost_J[j] = sum(abs(J_x0[j,:]-J_x[j,:]))*dx
	cost_n[j] = sum(abs(n_x0[j,:]-n_x[j,:]))*dx

	return n_x, V, Psi0, Psi1, J_x, cost_J[j], cost_n[j]
...
print 'Reading input...'
if int(TD) == 1:
	ProbPsi = open("ProbPsi(Nx=%s,Nt=%s).db" % (Nx,Nt), "r")			#Exact solution to the two electron system.
	n_x0[:,:] = pickle.load(ProbPsi)[:,:]						#Defines n_x0 from the exact wave-function.
	ProbPsi.close()
else:
	ProbPsi = open("ProbPsi(Nx=%s,Nt=%s).db" % (Nx,1), "r")				#Exact solution to the two electron system.
...
for j in range(Nt):
	...
	if j == 0:
		if int(selection) == 1:							#If the ground-state potential is known. 
			...
			V[j,:] = pickle.load(GSP)[:]
			GSP.close()
			n_x, Psi0, Psi1, V, cost_n[j], K, Psi2, Psi3 = Groundstate(V, n_x0, j, 1, 0, 1)
			print '--------------------------------------------------------------'
			print 'Ground-state cost = %s' % cost_n[j]
			print '--------------------------------------------------------------'
			f = open('Psi0.out', 'w')					#1D charge density.
			for i in range(Nx):
				f.write('%s %s\n' % (i*dx-0.5*L, Psi0[j,i].real))
			f.close()
			f = open('Psi1.out', 'w')					#1D charge density.
			for i in range(Nx):
				f.write('%s %s\n' % (i*dx-0.5*L, Psi1[j,i].real))
			f.close()
			f = open('Psi2.out', 'w')					#1D charge density.
			for i in range(Nx):
				f.write('%s %s\n' % (i*dx-0.5*L, Psi2[i].real))
			f.close()
			f = open('Psi3.out', 'w')					#1D charge density.
			for i in range(Nx):
				f.write('%s %s\n' % (i*dx-0.5*L, Psi3[i].real))
			f.close()
			if cost_n[j] > 1e-5:
				print 'Error: your ground-state Kohn-Sham potential is not accurate enough.'
				selection = 0
			print 'KS energy difference:', K[2], K[1], K[0]
		...
		if int(selection) == 0:							#If the ground-state potential is not 												known. 
			...
			for i in range(Nx):
				f.write('%s %s\n' % (i*dx-0.5*L, V[j,i]))
			f.close()
			n_x, Psi0, Psi1, V, cost_n[j], K, Psi2, Psi3 = Groundstate(V, n_x0, j, 1,\
			 0, 1)
			f = open('Den_non(t=%s).out' % j, 'w')				#1D charge density.
			for i in range(Nx):
				f.write('%s %s\n' % (i*dx-0.5*L, n_x[j,i]))
			f.close()
			n_x, Psi0, Psi1, V, cost_n[j], K, Psi2, Psi3 = Groundstate(V, n_x0, j, 1,\
			mu, 1)								#First guess is an LDA like potential.
			f = open('Psi0.out', 'w')					#1D charge density.
			for i in range(Nx):
				f.write('%s %s\n' % (i*dx-0.5*L, Psi0[j,i].real))
			f.close()
			...
			print
			print '--------------------------------------------------------------'
			print 'Initial guess cost = %s' % cost_n[j]			#Final cost for that step, has the form of 												the standard deffinition.
			print '--------------------------------------------------------------'
			print 
			...
			while cost_n[j] > noise:
				cost_old = cost_n[j]
				print cost_old
				n_x, Psi0, Psi1, V, cost_n[j], K, Psi2, Psi3 = \
				Groundstate(V, n_x0, j, 1, mu, 0)
		...
		if int(answ) == 1:
			V_func = zeros((Nt,Nx))
			V_func[j,:] = (0.25/n_x[j,:])*(gradient(gradient(n_x[j,:],\
			dx), dx))-(0.125/(n_x[j,:]**2))*(gradient(n_x[j,:], dx))**2
			for i in range(0, 4):
				V_func[j,i] = V_func[j,4]
			for i in range(Nx-5, Nx):
				V_func[j,i] = V_func[j,Nx-5]
			x0 = 0.5*L
			V[j,:] += V_func[j,int(((x0+0.5*L)/dx))]-\
			V[j,int(((x0+0.5*L)/dx))]
			f = open('V_func_GS.out', 'w')
			for i in range(Nx):
				f.write('%s %s\n' % (i*dx-0.5*L, V_func[j,i]))
			f.close()
			n_x, Psi0, Psi1, V_func, cost_n[j], K, Psi2, Psi3 = \
			Groundstate(V_func, n_x0, j, 1, 0, 1)
			V_h[j,:] = Hartree(n_x[j,:])					#Calculate the Hartree potential.
			for i in range(Nx):
				x = i*dx-0.5*L
				V_xc[j,i] = V_func[j,i]+(pm.well(x)+V_h[j,i])		#Take away the other potentials leaving 											only the exchange potential.
				V_xc_H[j,i] = V_func[j,i]-pm.well(x)			#Take away the external potential.

			f = open('Den_func(t=%s).out' % j, 'w')
			for i in range(Nx):
				f.write('%s %s\n' % (i*dx-0.5*L, n_x[j,i]))
			f.close()
			f = open('xc_func(t=%s).out' % j, 'w')
			for i in range(Nx):
				f.write('%s %s\n' % (i*dx-0.5*L, V_xc[j,i]))
			f.close()
			f = open('Hxc_func(t=%s).out' % j, 'w')
			for i in range(Nx):
				f.write('%s %s\n' % (i*dx-0.5*L, V_xc_H[j,i]))
			f.close()
			n_x, Psi0, Psi1, V, cost_n[j], K, Psi2, Psi3 = Groundstate\
			(V, n_x0, j, 1, 0, 1)
	else:
		t = j*dt
		print
		...
		#ufile = open('costREU(t=%s).out' % j, 'w')
		print
		print 'Applying vector correction...'
		n_x, V, cost_J[j], Psi0, Psi1, J_x, A, cost_n[j], cost_U[j]= \
		KohnSham(V, j, Psi0, Psi1, n_x0, J_x0, A, mmax, qmin, cost_U, n_x)
		Apot[:] = 0
		for i in range(Nx):
			for k in range(i+1):
		...
		a_F[j,:] = (U_F[j,:]-U_F[j-1,:])/dt+0.5*gradient(U_F[j,:]**2, dx)
		a_F0[j,:] = (U_F0[j,:]-U_F0[j-1,:])/dt+0.5*gradient(U_F0[j,:]**2, dx)
...
#-----------------------Isolating the exchange correlation potential-----------------
	...
	for i in range(Nx):
		...
		if j == 0:
			...
			V_xc[j,i] = V[j,i]-(pm.well(x)+V_h[j,i]+pm.petrb(x))
			V_xc_H[j,i] = V[j,i]-(pm.well(x)+pm.petrb(x))

#--------------------------Calculating confining electric field----------------------

	E[j,:] = -gradient(V[j,:], dx)							#The electric field is minus the gradient 												of the potential.
	E_xc[j,:] = -gradient(V_xc[j,:], dx)						#The electric field is minus the gradient 												of the potential.
	E_xc_H[j,:] = -gradient(V_xc_H[j,:], dx)					#The Hxc electric field.
	E_A_x[j,:] = -gradient(Apot[:], dx)						#The Hxc-Hxc(t=0) electric field.

	if j>0:
		E_A_t[j,:] = (-A[j,:]+A[j-1,:])/dt
...
#-----------------------------Output files & error checking---------------------------
	...
	for i in range(Nx):
		f.write('%s %s\n' % (i*dx-0.5*L, U_F0[j,i]))
	f.close()
	f = open('U_s0(t=%s).out' % j, 'w')						#Real velocity field.
	for i in range(Nx):
		f.write('%s %s\n' % (i*dx-0.5*L, U_s0[j,i]))
	f.close()
	f = open('U_s1(t=%s).out' % j, 'w')						#Real velocity field.
	for i in range(Nx):
		f.write('%s %s\n' % (i*dx-0.5*L, U_s1[j,i]))
	f.close()
	...
	for i in range(Nx):
		f.write('%s %s\n' % (i*dx-0.5*L, a_F0[j,i]))
	f.close()
	f = open('E_s(t=%s).out' % j, 'w')						#Real velocity field.
	for i in range(Nx):
		f.write('%s %s\n' % (i*dx-0.5*L, E_s[j,i]))
	f.close()
	...
	for i in range(Nx):
		f.write('%s %s\n' % (i*dx-0.5*L, Apot[i]))
	f.close()
	f = open('E_xc_H(t=%s).out' % j, 'w')						#Plot the Hxc electric field.
	for i in range(Nx):
		f.write('%s %s\n' % (i*dx-0.5*L, E_xc_H[j,i]))
	f.close()
	f = open('E_A_t(t=%s).out' % j, 'w')						#Plot the Hxc electric field.
	for i in range(Nx):
		f.write('%s %s\n' % (i*dx-0.5*L, E_A_t[j,i]))
	f.close()
	f = open('E_A_x(t=%s).out' % j, 'w')						#Plot the Hxc electric field.
	for i in range(Nx):
		f.write('%s %s\n' % (i*dx-0.5*L, E_A_x[j,i]))
	f.close()
	f = open('Psi0(t=%s).out' % j, 'w')						#Plot the Hxc electric field.
	for i in range(Nx):
		f.write('%s %s\n' % (i*dx-0.5*L, Psi0[j,i].real))
	f.close()
	f = open('Psi1(t=%s).out' % j, 'w')						#Plot the Hxc electric field.
	for i in range(Nx):
		f.write('%s %s\n' % (i*dx-0.5*L, Psi1[j,i].real))
	f.close()
efile.close()
...
if int(TD) == 1:
	...
	File = open("Currentdensity[0].db", "w")					#Writes the current density to file.
	pickle.dump(J_x0,File)
	File.close()

	File = open("Vectorpot.db", "w")						#Writes the vector potential to file.
	pickle.dump(A,File)
	File.close()

	File = open("Electric_time.db", "w")						#Writes the time dependent part of the 												electric field to file.
	pickle.dump(E_A_t,File)
	File.close()

	File = open("Velocityfield.db", "w")						#Writes the velocity field to file.
	pickle.dump(U_F0,File)
	File.close()
