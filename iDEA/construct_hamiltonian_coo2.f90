...
!        COOrdinate form of the system's Hamiltonian. These can then be constructed into  
...
!
!----------------------------------------------------------------------------------------!
!Created by Aaron James Long (al876@york.ac.uk) and Mike Entwistle (me624@york.ac.uk)    !
!________________________________________________________________________________________!


!========================================================================================!
! CONSTRUCT HAMILTONIAN OPERATOR IN SPARSE COOrdinate FORM (2 electrons)                 !
!----------------------------------------------------------------------------------------!
!
...
subroutine construct_hamiltonian_coo(coo_1, coo_2, coo_data, grid, constant, v_ext,& 
  ...
  integer            :: i, j, k, jk, band

  !======================================================================================!
  ! F2PY SIGNATURE CREATION                                                              !
  !--------------------------------------------------------------------------------------!
  !f2py intent(in) :: grid, max_size, bandwidth
  ...
  do j = bandwidth-1, grid-bandwidth
     do k = bandwidth-1, grid-bandwidth
        ...
        coo_1(i) = jk
        coo_2(i) = jk
        coo_data(i) = band_elements(0) + constant*potential(j, k, interaction_strength, v_ext, v_coulomb, grid)
        i = i+1
        !_______________________________________________________!
        ...
        do band = 1, bandwidth-1
            coo_1(i) = single_index(j+band, k, grid)
            coo_2(i) = jk
            ...
            coo_1(i) = single_index(j, k+band, grid)
            coo_2(i) = jk
            coo_data(i) = band_elements(band)
            i = i+1
...
!========================================================================================!
function potential(j, k, interaction_strength, v_ext, v_coulomb, grid) result(element)
    ...
    element = v_ext(j) + v_ext(k) + interaction_strength*v_coulomb(ABS(j-k))
