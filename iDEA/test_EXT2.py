...
class TestHarmonicOscillator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        pm.ext.itmax = 200.0              #: Total imaginary time
    ...
    def test_time_dependence(self):
        ...
        deltan = np.sum(np.absolute(den_td[100,:]-den_gs[:]))
        deltac = np.sum(np.absolute(cur[100,:]))

        nt.assert_allclose(deltan, 1.45e-3, atol=1e-5)
        nt.assert_allclose(deltac, 6.62e-2, atol=1e-4)
...
class TestDoubleWell(unittest.TestCase):                                                         
    ...
    def test_stencil_three(self):
        ...
        nt.assert_allclose(results.gs_ext_E, -1.70350, atol=1e-5)
    ...
    def test_stencil_five(self):                    
        ...
        nt.assert_allclose(results.gs_ext_E, -1.70086, atol=1e-5)
    ...
    def test_stencil_seven(self):                    
        ...
        nt.assert_allclose(results.gs_ext_E, -1.70075, atol=1e-5)
