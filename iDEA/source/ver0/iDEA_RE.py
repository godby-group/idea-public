...
def ReadInput(approx,n_MB,GS,imax):
    ...
    if pm.TD==1:
        if GS==0:
            ...
            # Then read im the time-dependnet density
...
def CalculateGroundstate(V_KS,n_MB,mu,sqdx,V_ext,T,n_KS):
    ...
    for i in range(pm.NE):
        n_KS[0,:]+=abs(U[:,i])**2 # Calculate the density from the single-particle wavefunctions
...
def SolveKSE(V_KS,A_KS,U_t,j,frac1,frac2,z):
    ...
    for i in range(pm.NE):
        ...
        U_t[z,:,i]=spla.spsolve(Mat,B)
        z=z*(-1)+1	

    return V_KS,A_KS,U_t,z
...
def CalculateKS(V_KS,A_KS,J_KS,U_t,j,upper_bound,frac1,frac2,z,tol,n_MB,J_MB,cost_n,cost_J,A_min,n_KS,Apot,exp):

    # Set initial trial vector potential as previous time-step's vector potential
    A_KS[j,:]=A_KS[j-1,:] 
    V_KS,A_KS,U_t,z=SolveKSE(V_KS,A_KS,U_t,j,frac1,frac2,z)

    # Calculate KS charge density
    n_KS[j,:]=0
    z=z*(-1)+1 # Only save two times at any point
    for i in range(pm.NE):
        n_KS[j,:]+=abs(U_t[z,:,i])**2
    z=z*(-1)+1
    ...
    while (count<=count_max): # Exit condition: KS and exact current density are equal at all points
        ...
        if count%10==0:
            ...
            if tol<1e-3: # Minimum accuracy limit
                ...
                V_KS,A_KS,U_t,z=SolveKSE(V_KS,A_KS,U_t,j,frac1,frac2,z) # Solve Schrodinger equation for KS system using initial trial potential
                n_KS[j,:]=0
                z=z*(-1)+1 # Only save two times at any point
                for i in range(pm.NE):
                    n_KS[j,:]+=abs(U_t[z,:,i])**2
        ...
        V_KS,A_KS,U_t,z=SolveKSE(V_KS,A_KS,U_t,j,frac1,frac2,z) # Solve KS equations using updated vector potential
        n_KS[j,:]=0
        z=z*(-1)+1 # Only save two times at any point
        for i in range(pm.NE):
            n_KS[j,:]+=abs(U_t[z,:,i])**2
        ...
        if count>=count_max:
            ...
            z=z*(-1)+1 # Only save two times at any point
            break
    string='REV: t = ' + str(j*pm.deltat) + ', tol = ' + str(tol) + ', electron current error = ' + str(cost_J[j]) + ', electron density error = ' + str(cost_n[j])
    sprint.sprint(string,1,1,pm.msglvl)
...
def main(approx):
    ...
    U_t=np.zeros((2,pm.jmax,pm.NE),dtype='complex')
    petrb=np.zeros(pm.jmax,dtype='float')

    # Begin
    n_MB=ReadInput(approx,n_MB,0,imax) # Read in exact charge density obtained from code
    V_KS,n_KS,U,V_ext=GroundState(V_KS,n_MB,mu,sqdx,V_ext,T,n_KS) # Calculate (or, if already obtained, check) ground-state KS potential
    ...
    for k in range(pm.NE):
        U_t[z,:,k]=U[:,k] # Initial set-up for time-dependence 
    ...
    if pm.TD==1:
        ...
        for j in range(1,imax): # Propagate from the ground-state
            n_KS,V_KS,J_KS,Apot,z=CalculateKS(V_KS,A_KS,J_KS,U_t,j,upper_bound,frac1,frac2,z,tol,n_MB,J_MB,cost_n,cost_J,A_min,n_KS,Apot,exp) # Calculate KS potential
