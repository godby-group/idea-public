...
Psi2 = zeros((imax,jmax), dtype='complex')
...
def TISE(V_KS,j):  					                         											
        ...
        Psi1[j,:] = U[:,1]/sqdx
        Psi2[j,:] = U[:,2]/sqdx
        n_x[j,:] = abs(Psi0[j,:])**2+abs(Psi1[j,:])**2+abs(Psi2[j,:])**2 # Calculate charge density				   
        return n_x[j,:], Psi0[j,:], Psi1[j,:], Psi2[j,:]  
...
def CrankNicolson(V_KS, Psi0, Psi1, Psi2, n, j): 
	...
	n[j,:] = abs(Psi0[j,:])**2+abs(Psi1[j,:])**2+abs(Psi2[j,:])**2
	return n, Psi0, Psi1, Psi2
...
V_ext[:] = V_KS[j,:] 
n_x[j,:], Psi0[j,:], Psi1[j,:], Psi2[j,:] = TISE(V_KS[j,:],j) # Solve Schrodinger Equation initially
n_x_old[j,:] = n_x[j,:]
while(Cost>tol):  
    ...
    n_x[j,:], Psi0[j,:], Psi1[j,:], Psi2[j,:] = TISE(V_KS[j,:],j) # Solve Schrodinger Equation
...
if(TD == 0): 
   f = open('outputs/' + str(pm.run_name) + '/raw/' + str(pm.run_name) + '_3gs_lda_vks.db', 'w') # KS potential	
   pickle.dump(V_KS[0,:],f)				
   f.close()
   f = open('outputs/' + str(pm.run_name) + '/raw/' + str(pm.run_name) + '_3gs_lda_vh.db', 'w') # H potential	
   pickle.dump(V_h[0,:],f)				
   f.close()
   f = open('outputs/' + str(pm.run_name) + '/raw/' + str(pm.run_name) + '_3gs_lda_vxc.db', 'w') # XC potential	
   pickle.dump(V_xc[0,:],f)				
   f.close()
   f = open('outputs/' + str(pm.run_name) + '/raw/' + str(pm.run_name) + '_3gs_lda_den.db', 'w') # Density	
   pickle.dump(n_x[0,:],f)				
   f.close()
...
if(TD==1):
   ...
   for j in range(1,imax): # Evolve TDSE using Crank-Nicolson scheme
       ...
       PS(string)
       n_x, Psi0, Psi1, Psi2 = CrankNicolson(V_KS[j,:], Psi0, Psi1, Psi2, n_x, j)  
       J_x[j,:] = Currentdensity(j,n_x)
       V_h[j,:] = Hartree(n_x[j,:]) 
       V_xc[j,:] = XC(n_x[j,:])
       if(j != imax-1):
           V_KS[j+1,:] = V_ext[:] + V_h[j,:] + V_xc[j,:] # Update KS potential
   f = open('outputs/' + str(pm.run_name) + '/raw/' + str(pm.run_name) + '_3td_lda_vks.db', 'w') # KS potential	
   pickle.dump(V_KS,f)				
   f.close()
   f = open('outputs/' + str(pm.run_name) + '/raw/' + str(pm.run_name) + '_3td_lda_vh.db', 'w') # H potential	
   pickle.dump(V_h,f)				
   f.close()
   f = open('outputs/' + str(pm.run_name) + '/raw/' + str(pm.run_name) + '_3td_lda_vxc.db', 'w') # XC potential	
   pickle.dump(V_xc,f)				
   f.close()
   f = open('outputs/' + str(pm.run_name) + '/raw/' + str(pm.run_name) + '_3td_lda_den.db', 'w') # Density	
   pickle.dump(n_x,f)				
   f.close()
   print
