...
def error(E_xc_LDA, E_xc_Exact): 
        if (E_xc_Exact != 0):
            ...
            print
            print('LDA: exchange-correlation energy: ' + str(round(E_xc_LDA, 4)))
...
if (TD == 0): 
   f = open('outputs/' + str(pm.run_name) + '/raw/' + str(pm.run_name) + '_2gs_lda_vks.db', 'w') # KS potential	
   pickle.dump(V_KS[0,:],f)				
   f.close()
   f = open('outputs/' + str(pm.run_name) + '/raw/' + str(pm.run_name) + '_2gs_lda_vh.db', 'w') # H potential	
   pickle.dump(V_h[0,:],f)				
   f.close()
   f = open('outputs/' + str(pm.run_name) + '/raw/' + str(pm.run_name) + '_2gs_lda_vxc.db', 'w') # XC potential	
   pickle.dump(V_xc[0,:],f)				
   f.close()
   f = open('outputs/' + str(pm.run_name) + '/raw/' + str(pm.run_name) + '_2gs_lda_den.db', 'w') # Density	
   pickle.dump(n_x[0,:],f)				
   f.close()
...
if (TD ==1):
   ...
   for j in range(1,imax): # Evolve TDSE using Crank-Nicolson scheme
       ...
       PS(string)
       n_x, Psi0, Psi1 = CrankNicolson(V_KS[j,:], Psi0, Psi1, n_x, j)  
       J_x[j,:] = Currentdensity(j,n_x)
       V_h[j,:] = Hartree(n_x[j,:]) 
       V_xc[j,:] = XC(n_x[j,:])
       if(j != imax-1):
           V_KS[j+1,:] = V_ext[:] + V_h[j,:] + V_xc[j,:] # Update KS potential
   f = open('outputs/' + str(pm.run_name) + '/raw/' + str(pm.run_name) + '_2td_lda_vks.db', 'w') # KS potential	
   pickle.dump(V_KS,f)				
   f.close()
   f = open('outputs/' + str(pm.run_name) + '/raw/' + str(pm.run_name) + '_2td_lda_vh.db', 'w') # H potential	
   pickle.dump(V_h,f)				
   f.close()
   f = open('outputs/' + str(pm.run_name) + '/raw/' + str(pm.run_name) + '_2td_lda_vxc.db', 'w') # XC potential	
   pickle.dump(V_xc,f)				
   f.close()
   f = open('outputs/' + str(pm.run_name) + '/raw/' + str(pm.run_name) + '_2td_lda_den.db', 'w') # Density	
   pickle.dump(n_x,f)				
   f.close()
   print
